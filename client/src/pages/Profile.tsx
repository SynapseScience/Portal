import React, { useState, useEffect } from "react";
import UserCard from "../parts/UserCard";

export default function Profile({ me, session, token }) {
  
  const [user, setUser] = useState(null);
  const [tried, setTried] = useState(false);
  let username = me ? me.username : null;
  const url = new URLSearchParams(window.location.search);
  if(url.has("username")) username = url.get("username");

  useEffect(() => {
    (async () => {
      const url = `${session.apiUrl}/user?username=${username}`
      const response = await fetch(url, {
        method: "GET"
      })
      
      if(response.ok) {
        const userInfos = await response.json();
        setUser(userInfos);
      }

      setTried(true);
      
    })()
  }, [username])

  return user ? <div className="cols" style={{ gap: "0px" }}>
    <div className="bubble">
      <UserCard user={user} me={me} session={session} token={token} />
    </div>
    <div className="bubble" style={{ paddingLeft: "0px" }}>
      <h1>Applications</h1>
      <p>Fonctionnalité en développement !</p>
      <h1>Favoris</h1>
      <p>Fonctionnalité soumise au référendum</p>
    </div>
  </div> : <div className="bubble">{ 
    tried && <>
      <h1>Utilisateur non trouvé /:</h1>
      <p>Conseil de pro : tous les noms d'utilisateur sont en minuscule !</p>
    </>
  }</div>
}